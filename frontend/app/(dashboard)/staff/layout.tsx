...
const navItems = [
  ...
  { href: "/staff/leads", label: "My Leads", icon: Target },
  { href: "/staff/followups", label: "Follow-ups", icon: PhoneCall },
  { href: "/staff/customers", label: "Customers", icon: UserCircle },
  { href: "/staff/quotations", label: "Quotations", icon: FileText },
  { href: "/staff/location", label: "Location", icon: MapPin },
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 rounded-lg px-3 py-2 transition-all ${
                  isActive ? "bg-blue-600/10 text-blue-500" : "hover:bg-slate-800 hover:text-white"
                }`}
              >
                <item.icon className={`h-4 w-4 ${isActive ? "text-blue-500" : "text-slate-400"}`} />
                {item.label}
              </Link>
            )
          })}
